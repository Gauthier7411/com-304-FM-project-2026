...
class VQACollator(object):  # Visual Question Answering Collator
    ...
    def __call__(self, batch):
        ...
        original_lengths = [
            len(self.tokenizer.encode(sequence, add_special_tokens=False))
        ...
        for i in range(len(batch)):
            ...
            first_token_pos = attention_mask[i].argmax().item() # the first 1 (non-zero value) in the attention mask for a given batch sample marks the first non-padding token
